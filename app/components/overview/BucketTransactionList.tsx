    ...
    <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
      {transactions.map((t) => (
        <div key={t.id} className="flex items-center gap-2 px-4 py-2.5 text-sm">
          <span className="w-8/100 shrink-0 text-zinc-500 dark:text-zinc-400">{formatDate(t.date)}</span>
          <span className="w-42/100 shrink-0 truncate text-zinc-500 dark:text-zinc-400">{t.description}</span>
          <span className="w-14/100 shrink-0 truncate text-xs text-zinc-400 dark:text-zinc-500">{t.account.name}</span>
          <div className="w-22/100 shrink-0">
          ...
          <span className="w-12/100 shrink-0 text-right">
