...
const log = logger.child({ cat: "auth" });
...
router.post("/register", registerLimiter, async (req, res) => {
  try {
    ...
    if (!email || !password || password.length < 6 || password.length > 72) {
      log.warn("Register failed: invalid input");
      res.status(400).json({ error: "Email and password (6-72 chars) required" });
      return;
    }
    if (!EMAIL_RE.test(email)) {
      log.warn("Register failed: invalid email format");
    ...
    if (existing) {
      log.warn("Register failed: duplicate email");
...
router.post("/login", loginLimiter, loginByEmailLimiter, async (req, res) => {
  try {
    ...
    if (!EMAIL_RE.test(email)) {
      log.warn("Login failed: invalid email format");
      res.status(400).json({ error: "Invalid email format" });
      return;
    }
...
router.delete("/account", deleteAccountLimiter, requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (!password || typeof password !== "string") {
      log.warn("Delete account failed: password required");
    ...
    if (!user || !(await bcrypt.compare(password, user.passwordHash))) {
      log.warn("Delete account failed: invalid password");
    ...
    log.info({ plaidItemsRevoked: items.length }, "Account deleted");
